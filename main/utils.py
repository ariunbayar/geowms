...
def get_1stOrder_geo_id():
    ...
    feature_id = LFeatures.objects.filter(feature_code='au-au-au').first()
    if feature_id:
        feature_id = feature_id.feature_id

    property_id = LProperties.objects.filter(property_code='NationalLevel').first()
    if property_id:
        property_id = property_id.property_id

    code_list_id = LCodeLists.objects.filter(code_list_code='1stOrder\n').first()
    if code_list_id:
        code_list_id = code_list_id.code_list_id
    ...
    qs = qs.filter(feature_config_id__in=feature_config_ids).first()
