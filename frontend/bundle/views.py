...
def wms_layers(request, pk):
    ...
    def _layer_to_display(layer):
        qs = BundleLayer.objects.filter(bundle_id=pk, layer_id=layer.pk, role_id=role)
        return {
            'name': layer.name,
            'code': layer.code,
            'defaultCheck': qs.values('defaultCheck')[0]['defaultCheck']
        }
