...
def _get_package(theme_id):
    package_data = []
    for package in LPackages.objects.using('geoportal').filter(theme_id=theme_id):
        package_data.append({
                'id': package.package_id,
                'code': package.package_code,
                'name': package.package_name,
                'features': list(LFeatures.objects.using('geoportal').filter(package_id=package.package_id).extra(select={'id': 'feature_id', 'code': 'feature_code', 'name': 'feature_name'}).values('id', 'code', 'name'))
            })
    return package_data
...
def bundleButetsAll(request):
    data = []
    for themes in LThemes.objects.using('geoportal').all():
        data.append({
                'id': themes.theme_id,
